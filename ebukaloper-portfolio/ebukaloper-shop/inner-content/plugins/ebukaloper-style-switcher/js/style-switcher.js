...
var EBUKALOPER_SS = EBUKALOPER_SS || {};
...
(function($){
	...
	EBUKALOPER_SS.styleSwitcher = function(options){
		this.options = options;
	};

	ss = EBUKALOPER_SS.styleSwitcher.prototype;
	...
	ss.buildSwitcher = function(){
		var self = this,
			...
			opened = sessionStorage.ebukaloper_ss_opened === undefined ? self.options.switcher_opened :
				JSON.parse(sessionStorage.ebukaloper_ss_opened),
			$blocks;

		self.$el = $('<div />', {'class':'ebukaloper-style-switcher'})
			.prependTo($parent);
	...
	ss.toggleSwitcher = function(open, animate){
		var self = this,
			...
			doOnComplete = function(){
				...
				self.opened = open;
				sessionStorage.ebukaloper_ss_opened = open;
			};
	...
	ss.addColorOptions = function(){
		...
		if(len){
			for(i=0; i<len; i++){
				...
				new EBUKALOPER_SS.colorSetter(id, color, self.$el, self.blockSize).init();
			}
		}
	};	
	...
	ss.addPatternOptions = function(){
		...
		if(len){
			for(i=0; i<len; i++){
				...
				new EBUKALOPER_SS.patternSetter(id, pattern, self.$el, self.blockSize).init();
			}
		}
	};
	...
	ss.addSelectOptions = function(){
		...
		if(len){
			for(i=0; i<len; i++){
				...
				new EBUKALOPER_SS.selectSetter(id, select, $parent).init();
			}
		}
	};	
	...
	EBUKALOPER_SS.patternSetter = function(id, options, $parent, size){
		this.itemId = id;
		this.options = options;
		this.$parent = $parent;
		this.elSize = size;
	};
	ps = EBUKALOPER_SS.patternSetter.prototype;
	...
	EBUKALOPER_SS.colorSetter = function(id, options, $parent, size){
		this.itemId = id;
		this.options = options;
		this.$parent = $parent;
		this.elSize = size;
	...
	cs = EBUKALOPER_SS.colorSetter.prototype;
	...
	EBUKALOPER_SS.selectSetter = function(id, options, $parent){
		this.itemId = id;
	...
	var ssel = EBUKALOPER_SS.selectSetter.prototype;
