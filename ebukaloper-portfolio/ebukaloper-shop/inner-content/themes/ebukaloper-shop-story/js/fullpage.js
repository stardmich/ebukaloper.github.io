...
"use strict";
	 ...
	 */
	EBUKALOPER.Fullpage = function(selector, options){
		// return;
		var defaults = {
	...
	EBUKALOPER.Fullpage.prototype = {
		...
		init : function(){
			...
			if(this.secNum){
				...
				this.$win = $(window);
				this.slides = [];
				this.mobile = EBUKALOPER.utils.checkIfMobile();
		...
		loadSlider : function(){
			...
			this.loader = new EBUKALOPER.Loader(this.$sections, {bgElements : '.slide'});
		...
		doOnLeave : function(index, nextIndex, direction){
			...
			if($video.length && EBUKALOPER.supportsVideo()){
				$video.get(0).pause();
			}
	...
	EBUKALOPER.Loader = function($elements, options){
		var defaults = {
			loadImg : true,
			loadBg : true,
			onAllLoaded : null
	...
	EBUKALOPER.Loader.prototype = {
		...
		_loadElement : function(index){
			...
			$(imagesToLoad).ebukaloperOnImgLoaded({callback:function(){
				// setTimeout(function(){
					imagesLoaded = true;
			...
			if(videoNumToLoad){
				videosToLoad.each(function(){
					...
					if(video.readyState==4){
						...
						if(!EBUKALOPER.getBrowser().chrome){
							//reload the video
							video.load();
						}
